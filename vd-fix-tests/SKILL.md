---
name: vd-fix-tests
description: Run tests, identify failures, and fix them with context from recent commits. Use when tests are failing after changes and need automated fixing with commit context.
---

# Fix Tests

Run tests to identify failures, then fix them using context from recent commits. This command helps maintain test health after implementing new features or fixes.
...
1. Run `make test` (or appropriate test command) to execute the full test suite
